...
interface GameStore extends GameState {
  ...
  placeSelectedWords: (categoryId: string) => boolean; // returns true if correct
...
// Re-export types for convenience
  ...
  selectWord: (wordId: string) => {
    ...
    // If 4 words already selected, deselect the oldest
  ...
  // Remove word from category (send back to pool)
  ...
  submitCategory: (categoryId: string) => {
    ...
    // Check if all words in this category are correct
